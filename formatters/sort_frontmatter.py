    ...
    while i < len(lines):
        ...
        if ':' in line and not line.startswith(' ') and not line.startswith('\t'):
            ...
            # Kontrollera om nästa rad är en lista
                ...
                next_line = lines[i + 1]
                if next_line.strip().startswith('-') or (current_property == 'andringsforfattningar' and next_line.startswith(' ')):
                    in_list = True
...
def sort_frontmatter_in_directory(directory_path: str, pattern: str = "*.md", backup: bool = True) -> Dict[str, bool]:
    """
...
if __name__ == "__main__":
    ...
    if input_path.is_file():
        ...
        results = sort_frontmatter_in_directory(str(input_path), args.pattern, backup=not args.no_backup)
