...
def determine_output_path(data: Dict[str, Any], output_dir: Path,
                          year_as_folder: bool = True) -> Path:
    """
...
def make_document(data: Dict[str,
                             Any],
                  output_dir: Path,
                  output_modes: List[str] = None,
                  year_as_folder: bool = True,
                  verbose: bool = False,
                  git_mode: bool = False,
                  fetch_predocs_from_api: bool = False,
    ...
    if target_date is None and 'md' in output_modes:
        ...
        print(
            f"Varning: Använder dagens datum ({target_date}) som target_date för 'md' som output format")
...
def _create_markdown_document(data: Dict[str,
                                         Any],
                              output_path: Path,
                              git_mode: bool = False,
                              preserve_selex_tags: bool = False,
                              verbose: bool = False,
                              fetch_predocs_from_api: bool = False,
                              apply_links: bool = False,
    ...
    # Apply temporal processing to handle selex attributes (only if not in git
    # mode and not preserving selex tags)
    if not git_mode and not preserve_selex_tags and target_date:
        ...
        # Kontrollera om dokumentet är tomt efter temporal processing och lägg
        # till förklarande meddelande
        if is_document_content_empty(markdown_content):
            ...
            if verbose:
                print(
                    f"Info: Tomt dokument efter temporal processing för {beteckning}, lade till förklarande meddelande")
    ...
    if verbose:
        ...
        if len(markdown_content) < 1000:  # If suspiciously short, show preview
            print(
                f"Debug: Innehållsförhandsvisning eftersom misstänkt kort:\n{markdown_content[:500]}...")
            ...
            if parsed_predocs:
                if fetch_predocs_from_api:
                    ...
                    try:
                        ...
                        print(
                            f"Varning: Kunde inte hämta detaljerad förarbeten-information från API: {e}")
                        # Fallback to parsed data without API details
                        formatted_predocs = [
                            f"{p['type'].upper()} {p['rm']}:{p['bet']}" for p in parsed_predocs]
                else:
    ...
    if should_use_ignored_body and ignored_body is not None:
        ...
        if data.get('ikraftDenDagenRegeringenBestammer'):
            article_attributes.append(
                f'selex:ikraft_villkor="Denna lag träder i kraft den dag regeringen bestämmer."')

        # Check for conditional expiration
        if data.get('upphavdDenDagenRegeringenBestammer'):
            article_attributes.append(
                f'selex:upphor_villkor="Denna lag upphör att gälla den dag regeringen bestämmer."')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Process SFS documents from JSON to various output formats.')
