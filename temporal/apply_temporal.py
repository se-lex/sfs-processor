...
def apply_temporal(markdown_text: str, target_date: str, verbose: bool = False) -> str:
    ...
    try:
        ...
        raise ValueError(
            f"target_date måste vara i format YYYY-MM-DD, fick: {target_date}") from exc
    ...
    while i < len(lines):
        ...
        if section_match:
            ...
            if should_remove:
                ...
                # Om status="ikraft" med ikraft_villkor (utan specifikt datum), ta bort
                # temporal attribut
                if status_match and "ikraft" in status_match.group(
                        1) and ikraft_villkor_match and not ikraft_match and not should_clean_attributes:
                    should_clean_attributes = True
...
def add_empty_document_message(markdown_content: str,
    ...
    if data:
        ...
        if ikraft_datum and not isinstance(ikraft_datum, str):
            ikraft_datum = ikraft_datum.strftime(
                '%Y-%m-%d') if hasattr(ikraft_datum, 'strftime') else str(ikraft_datum)
        if upphavd_datum and not isinstance(upphavd_datum, str):
            upphavd_datum = upphavd_datum.strftime(
                '%Y-%m-%d') if hasattr(upphavd_datum, 'strftime') else str(upphavd_datum)
        if utgar_datum and not isinstance(utgar_datum, str):
            utgar_datum = utgar_datum.strftime(
                '%Y-%m-%d') if hasattr(utgar_datum, 'strftime') else str(utgar_datum)

        # Försök extrahera andringsforfattningar från frontmatter
        andringsforfattningar_str = extract_frontmatter_property(
            markdown_content, 'andringsforfattningar')
    ...
    if not message:
        if upphavd_datum or utgar_datum:
            ...
            for amendment in andringsforfattningar:
                ...
                if anteckningar and ('upphävd' in anteckningar.lower()
                                     or 'upphör' in anteckningar.lower()):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print(
            "Användning: python temporal/apply_temporal.py <markdown_fil> <target_date> [output_fil] [--verbose]")
