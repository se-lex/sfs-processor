...
def fetch_document_info(doc_type: str, rm: str, bet: str, max_retries: int = 3,
                        delay: float = 0.5) -> Optional[Dict[str, str]]:
    """
...
def fetch_document_ids(year: Optional[int] = None) -> List[str]:
    ...
    try:
        ...
        if year is not None:
            ...
            print(
                f"Filtrerade för år {year}: {len(document_ids)} av {original_count} författningar")
