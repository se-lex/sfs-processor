...
def analyze_ikraft_years(json_dir):
    ...
    for json_file in json_path.glob('*.json'):
        try:
            ...
            # Check main document
            if data.get('ikraftOvergangsbestammelse') and data.get('ikraftDateTime') is None:
            ...
            if 'andringsforfattningar' in data:
                for andring in data['andringsforfattningar']:
                    if andring.get('ikraftOvergangsbestammelse') and andring.get(
                            'ikraftDateTime') is None:
