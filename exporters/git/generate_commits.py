...
def create_init_git_commit(
    ...
    if is_document_content_empty(temporal_content):
        ...
        if verbose:
            print(
                f"Info: Tomt dokument efter temporal processing för {beteckning} vid {utfardad_datum}, lade till förklarande meddelande")
    ...
    temporal_content_with_rubrik = set_prop_in_frontmatter(
        temporal_content, "rubrik", temporal_rubrik)

    # Add ikraft_datum to frontmatter (even if it's a future date)
    ikraft_datum = format_datetime(data.get('ikraftDateTime'))
    if ikraft_datum:
        temporal_content_with_ikraft = set_prop_in_frontmatter(
            temporal_content_with_rubrik, "ikraft_datum", ikraft_datum)
    else:
        temporal_content_with_ikraft = temporal_content_with_rubrik

    # Remove andringsforfattningar from frontmatter in git mode
    temporal_content_clean = remove_prop_from_frontmatter(
        temporal_content_with_ikraft, "andringsforfattningar")
...
def generate_descriptive_commit_message(
    ...
    for change in changes:
        ...
        if change['type'] == 'ikraft':
            ...
            raise ValueError(
                f"Okänd ändringstyp '{change['type']}' för {section_id}. Kända typer: 'ikraft', 'upphor', 'upphor_villkor'")
    ...
    if has_ikraft and has_upphor:
        ...
        if message_parts:
            ...
            raise ValueError(
                f"Ikraft- och upphör-ändringar på samma datum, borde inte vara möjligt för {doc_name}. Kontrollera ändringarna.")
        ...
        if ikraft_sections:
            ...
            raise ValueError(
                f"Ikraft-ändringar hittades för {doc_name} men varken sections eller article-ändringar kunde identifieras")
        ...
        if upphor_sections:
            if len(upphor_sections) == 1:
                ...
                if set(upphor_sections).issubset(set(upphavd_sections)):
                    ...
                    # Mixed or temporal expiration - use general term but indicate if some are
                    # actively revoked
                    if upphavd_sections:
                        message = f"{emoji} {doc_name}: {sections_str} upphävs"
                    else:
            ...
            raise ValueError(
                f"Upphor-ändringar hittades för {doc_name} men varken sections eller article-ändringar kunde identifieras")
...
def generate_temporal_commits(
    ...
    if '<article' not in content:
        raise ValueError(
            f"Inga selex-taggar hittades i {markdown_file}. Temporal processing kräver att selex-taggar är kvar i dokumentet")
        ...
        for date in sorted(changes_by_date.keys()):
            ...
            try:
                ...
                if is_document_content_empty(filtered_content):
                    filtered_content = add_empty_document_message(
                        filtered_content, data=None, target_date=date)

                # Apply temporal title processing for frontmatter rubrik if it exists
                if rubrik:
                    temporal_rubrik = title_temporal(rubrik, date)
                    filtered_content = set_prop_in_frontmatter(
                        filtered_content, "rubrik", temporal_rubrik)

                # Remove andringsforfattningar from frontmatter in git mode
                filtered_content = remove_prop_from_frontmatter(
                    filtered_content, "andringsforfattningar")
