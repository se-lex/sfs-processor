    ...
    try:
        ...
        if total_files > batch_size:
            ...
            for i, batch in enumerate(batches, 1):
                ...
                _process_temporal_batch_files(batch, from_date, to_date, verbose, original_cwd, i, len(batches))
                ...
                if push_to_target_repository(unique_branch, 'origin', verbose):
                    print(f"Temporal batch {i}/{len(batches)} pushad till target repository som branch '{unique_branch}'")
                else:
                    print(f"Misslyckades med att pusha temporal batch {i}/{len(batches)} till target repository")
        else:
            ...
            if push_to_target_repository(unique_branch, 'origin', verbose):
                print(f"Alla {total_files} temporal commits pushade till target repository som branch '{unique_branch}'")
            else:
